...
const KeyFeatures = ({ project }: { project: Project }) => {
  return (
    <div className="overflow-hidden rounded-xl bg-[#161B22] ring-1 ring-white/10">
      <div className="flex items-center justify-between border-b border-white/10 md:p-6 p-3">
      ...
      <div className="grid gap-px bg-white/5">
        {project.keyFeatures.map((feature, index) => (
          <div
            key={index}
            className="group bg-[#161B22] md:p-6 p-3 transition-colors hover:bg-[#1C2128]"
          >
            <div className="mb-2 flex items-center gap-3">
              ...
              <h3 className="md:text-lg text-base font-medium text-white">{feature.title}</h3>
            </div>
            <p className="text-gray-400 pl-9 md:text-base text-sm ">{feature.description}</p>
          </div>
        ))}
      </div>
    </div>
